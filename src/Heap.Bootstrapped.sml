...
struct
  ...
  fun restore (xs, nil) = xs
    | restore (xs, (p, y) :: ys) =
      case bump xs of
          nil => restore ((p, y) :: xs, ys)
        | xs => restore (xs, (p, y) :: ys)
  
  fun merge2 (xs, nil, zs) = restore (xs, zs)
    | merge2 (nil, ys, zs) = restore (ys, zs)
    | merge2 ((p, x) :: xs, (q, y) :: ys, zs) =
      if Int.<= (p, q) then merge2 (xs, (q, y) :: ys, (p, x) :: zs)
                       else merge2 ((p, x) :: xs, ys, (q, y) :: zs)
  
  fun merge3 (xs, ys, nil, ws) = merge2 (xs, ys, ws)
    | merge3 (xs, nil, zs, ws) = merge2 (xs, zs, ws)
    | merge3 (nil, ys, zs, ws) = merge2 (ys, zs, ws)
    | merge3 ((p, x) :: xs, (q, y) :: ys, (r, z) :: zs, ws) =
      if Int.<= (p, q) andalso Int.<= (p, r) then
        merge3 (xs, (q, y) :: ys, (r, z) :: zs, (p, x) :: ws)
      else if Int.<= (q, r) then
        merge3 ((p, x) :: xs, ys, (r, z) :: zs, (q, y) :: ws)
      else
        merge3 ((p, x) :: xs, (q, y) :: ys, zs, (r, z) :: ws)
    ...
    case focus (xs, nil) of
      ...
      | SOME xs =>
        let
          ...
          val x ::: xs = x
        in
          SOME (x ::: merge3 (xs, ys, rev ws, nil))
        end
end
