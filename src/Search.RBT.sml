...
struct
   ...
   *)
  datatype 'a step = L of 'a * 'a tree | R of 'a tree * 'a | B
  
  fun upd (a, L (x,b) :: ss) = upd (Red (a,x,b), ss)
    | upd (b, R (a,x) :: ss) = upd (Red (a,x,b), ss)
    | upd (Red xs, B :: ss) = upd (Black xs, ss)
    | upd (xs, _) = xs
  ...
  (*  Refocus on the rightmost leaf.  *)
  fun cut (Red (a,x,b), ss) = cut (b, R (a,x) :: ss)
    | cut (_, ss) = ss
  
  (*  Rotate red nodes away from the rightmost path.  *)
  ...
  (*  Refocus on the leftmost leaf.  *)
  fun cut (Red (a,x,b), ss) = cut (a, L (x,b) :: ss)
    | cut (_, ss) = ss
  
  (*  Rotate red nodes away from the leftmost path.  *)
  ...
  fun loop (_, nil) = NONE
    ...
    | loop (r, Many (Red (a,x,b)) :: ss) = loop (r, Many a :: One x :: Many b :: ss)
    | loop (r, Many (Black (a,x,b)) :: ss) = loop (r, Many a :: One x :: Many b :: ss)
  ...
  fun loop (_, nil) = NONE
    | loop (r, One x :: ss) = SOME x before r := ss
    | loop (r, Many Empty :: ss) = loop (r, ss)
    | loop (r, Many (Red (a,x,b)) :: ss) = loop (r, Many b :: One x :: Many a :: ss)
    | loop (r, Many (Black (a,x,b)) :: ss) = loop (r, Many b :: One x :: Many a :: ss)
