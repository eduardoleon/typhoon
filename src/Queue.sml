...
struct
  ...
  (*  When the schedule is empty, the front is fully forced and has the same
   *  length as the rear, so we start the next rotation.  Conveniently enough,
   *  the empty schedule itself can be used as the new accumulator.
   *)
  fun exec (args as (xs, ys, r)) =
    case force r of
        zs as _ ::: _ => (xs, ys, zs)
      | _ => let val xs = step args in (xs, nil, xs) end
...
struct
  ...
  fun run (ws, Empty, Snoc x :: ss) = run (ws, pure x, ss)
    | run (ws, Queue (m, xs, ys, n, zs), Snoc z :: ss) =
      run (ws, Queue (m, xs, ys, n + 1, z :: zs), Rear1 :: ss)
    
    | run (ws, Queue (m, x :: xs, ys, n, zs), Uncons :: ss) =
      run (x :: ws, Queue (m - 1, xs, ys, n, zs), Rear1 :: Front1 :: ss)
    
    (*  The rear length invariant has higher priority than the nonempty
     *  front invariant in the sense that, if neither invariant holds,
     *  then the rear length invariant must be restored first.
     *)
    | run (ws, xyzs as Queue (m, xs, ys, n, zs), Rear1 :: ss) =
      if m >= n then
        run (ws, xyzs, ss)
      else
        run (ws, ys, Snoc (delay zs) :: Rear2 (m + n, xs) :: ss)
    
    | run (ws, ys, Rear2 (m, xs) :: ss) = run (ws, front (m, xs, ys), ss)
    
    (*  When we enforce the nonempty front invariant, we may assume that
     *  the rear length invariant already holds.  In particular, if the
     *  front and middle are empty, then so is the rear.
     *)
    | run (ws, xyzs as Queue (m, xs, ys, n, zs), Front1 :: ss) =
      let in
        case (xs, ys) of
            (nil, Empty) => run (ws, Empty, ss)
          | (nil, _) => run (ws, ys, Uncons :: Front2 (m, n, zs) :: ss)
          | _ => run (ws, xyzs, ss)
      end
    
    | run (Node r :: ws, ys, Front2 (m, n, zs) :: ss) =
      run (ws, Queue (m, force r, ys, n, zs), ss)
    
    | run args = args
  
  fun snoc (xs, x) = #2 (run (nil, xs, Snoc (Leaf x) :: nil))
  
  fun uncons xs =
    case run (nil, xs, Uncons :: nil) of
        (Leaf x :: _, xs, _) => SOME (x, xs)
      | _ => NONE
end
