...
struct
  ...
  datatype 'a try
    ...
    | Digit of 'a digit * 'a step list
    | Force of int * 'a seq ref * 'a step list
    | Seq of 'a seq * 'a step list
  
  type 'a hole = 'a step list
  
  fun find (n, Force (m, r, ss) :: ts) =
      if n < m then
        let val xs = force (!r)
    ...
    | find (n, Seq (Many (m, xs, ys, zs), ss) :: _) =
      find (n, Digit (xs, L (m, ys, zs) :: ss) ::
               Force (m, ys, M (xs, zs) :: ss) ::
               Digit (zs, R (m, xs, ys) :: ss) :: nil)
    
    | find (n, Digit (F1 a, ss) :: ts) =
      find (n, Tree (a, T11 :: ss) :: ts)
    
    | find (n, Digit (F2 (a,b), ss) :: ts) =
      find (n, Tree (a, T21 b :: ss) ::
               Tree (b, T22 a :: ss) :: ts)
    
    | find (n, Digit (F3 (a,b,c), ss) :: ts) =
      find (n, Tree (a, T31 (b,c) :: ss) ::
               Tree (b, T32 (a,c) :: ss) ::
               Tree (c, T33 (a,b) :: ss) :: ts)
    
    | find (n, Digit (F4 (a,b,c,d), ss) :: ts) =
