...
struct
  ...
  fun gabow (graph, roots) =
    let
      ...
      val size = Vector.length graph
      val index = IntArray.array (size, ~1)
      ...
      fun run (n, xs, ys, zs, Visit nil :: ss) = run (n, xs, ys, zs, ss)
        | run (n, xs, ys, zs, Visit (k :: ks) :: ss) =
          let in
            case IntArray.sub (index, k) of
                ~1 =>
                let
                  val js = Vector.sub (graph, k)
                in
                  IntArray.update (index, k, n);
        ...
        (*  If j's index is > c, then j isn't the earliest visited node in
         *  its strongly connected component.
         *)
        | run (n, xs, ys, zs as j :: js, Prune c :: ss) =
          if IntArray.sub (index, j) > c then
    ...
    let
      ...
      fun run (n, xs, ys, Visit k :: ss) =
        ...
        | run (n, xs, ys, Iter (k, c, nil) :: ss) =
          if IntArray.sub (index, k) = c then
            run (n, xs, ys, Collect (k, nil) :: ss)
          else
            let in
              IntArray.update (index, k, c);
              run (n, xs, ys, ss)
            end
